 ...
 */
final class Slug_Handler_Option implements Gedmo_Annotation
{
     ...
     * @param mixed                $value
     */
    public function __construct(array $data = [], string $name = '', $value = null)
    {
        if ([] !== $data) {
            ...
            $args = func_get_args();
            $this->name = $this->get_attribute_value($data, 'name', $args, 1, $name);
            $this->value = $this->get_attribute_value($data, 'value', $args, 2, $value);
            return;
        }
        $this->name = $name;
        $this->value = $value;
